...
### remove any old plugin jar
```bash
kubectl exec -n sonarqube sonarqube-sonarqube-0 -c sonarqube -- bash -c 'mv   "/opt/sonarqube/extensions/plugins/sonar-groovy-plugin-2025.1.0.jar" "/opt/sonarqube/extensions/plugins/sonar-groovy-plugin-2025.1.0.jar.bak"'
```

### Copy the plugin jar to the plugins dir
```bash
kubectl cp sonar-groovy-plugin/target/sonar-groovy-plugin-2025.2.0.jar sonarqube/sonarqube-sonarqube-0:"/opt/sonarqube/extensions/plugins/" -c sonarqube
```

### Verify the plugin jar
```bash
ls -al  sonar-groovy-plugin/target/sonar-groovy-plugin-2025.2.0.jar
kubectl exec -n sonarqube sonarqube-sonarqube-0 -c sonarqube -- bash -c 'ls -al /opt/sonarqube/extensions/plugins/sonar-groovy-plugin-2025.2.0.jar'
```
